...
public class CipherOps {
    ...
    private static final byte[][] INVSBOX = {
            { (byte) 0x52,        0x09,        0x6a, (byte) 0xd5,        0x30,        0x36, (byte) 0xa5,        0x38, (byte) 0xbf,        0x40, (byte) 0xa3, (byte) 0x9e, (byte) 0x81, (byte) 0xf3, (byte) 0xd7, (byte) 0xfb } ,
    ...
    private static final byte[][] INVAX = {
            { 0x0e, 0x0b, 0x0d, 0x09 },
            { 0x09, 0x0e, 0x0b, 0x0d },
            { 0x0d, 0x09, 0x0e, 0x0b },
            { 0x0b, 0x0d, 0x09, 0x0e }
    ...
    public static byte[] inverseSubBytes(byte[] w) {

        for (int i = 0; i < w.length; i++) {
            int[] nibbles = getNibbles(w[i]);
            w[i] = INVSBOX[nibbles[0]][nibbles[1]];
        }

        return w;
    }

    public static byte[][] inverseShiftRows(byte[][] s) {

        for (int i = 1; i < 4; i++) {
            s = inverseShiftRow(s, i, i);
        }

        return s;
    }

    private static byte[][] inverseShiftRow(byte[][] s, int row, int shiftCount) {
    ...
    public static byte[] inverseMixColumns(byte[] w) {

        byte[] resultArray = new byte[4];

        for (int i = 0; i < w.length; i++) {
            byte result = FiniteFieldArithmetic.ffMultiply(w[0], INVAX[i][0]);

            for (int j = 1; j < INVAX[i].length; j++) {
                result = FiniteFieldArithmetic.ffAdd(result, FiniteFieldArithmetic.ffMultiply(w[j], INVAX[i][j]));
            }
            resultArray[i] = result;
        }
        return resultArray;
